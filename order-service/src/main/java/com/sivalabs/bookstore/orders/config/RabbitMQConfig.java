...
class RabbitMQConfig {
    ...
    private final ApplicationProperties properties; // To load the application properties
    ...
    DirectExchange exchange() {
        return new DirectExchange(properties.orderEventsExchange()); // getting these fields from properties file
    }

    @Bean
    Queue newOrdersQueue() {
        return QueueBuilder.durable(properties.newOrdersQueue()).build(); // getting these fields from properties file
    }
    ...
    Binding errorOrdersQueueBinding() {
        return BindingBuilder.bind(errorOrdersQueue())
                .to(exchange())
                .with(properties.errorOrdersQueue()); // we are setting the routing key here e.g. Any text
        // .with("random-routing-key-name");
    }
